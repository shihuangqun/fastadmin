...
use think\Request;
use think\Validate;
use app\common\model\ConfLevel;
...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'avatar', 'score','level_user','level_charm'];
    ...
    public function init($token)
    {
        ...
        if ($user_id > 0) {
            ...
            //用户的等级
            $ConfLevel = new ConfLevel();
            $user->level_user = $ConfLevel->where('id',$user->level_user_id)->find();
            $user->level_charm = $ConfLevel->where('id',$user->level_charm_id)->find();
